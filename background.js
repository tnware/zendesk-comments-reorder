...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if ("autoProcessEnabled" in request) {
    // Enable or disable the automatic process based on the toggle value
    if (request.autoProcessEnabled) {
          ...
          if (element) {
            ...
            listContainers.forEach((listContainer) => {
              ...
              listItems = listItems.filter(
                (item) =>
                  item.getAttribute("data-test-id") !== "convo_log_sentinel_2"
              );
              listItems = listItems.filter(
                (item) =>
                  item.getAttribute("data-test-id") !== "convo_log_sentinel_1"
              );
